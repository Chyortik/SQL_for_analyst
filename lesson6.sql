...
DROP PROCEDURE IF EXISTS sp_data_analysis_cursor;
DELIMITER //
CREATE PROCEDURE sp_data_analysis_cursor(start_date DATE)
BEGIN

	DECLARE id_users INT;
	Declare done integer default 0;
	Declare UsersCursor Cursor for Select user_id FROM profiles WHERE birthday > start_date;
    DECLARE CONTINUE HANDLER FOR SQLSTATE '02000' SET done=1;
    Open UsersCursor;
	WHILE done = 0 DO
		FETCH UsersCursor INTO id_users;
		UPDATE profiles
			SET birthday=NOW()
...
-- вызов процедуры
CALL sp_data_analysis_cursor('2000-01-01');